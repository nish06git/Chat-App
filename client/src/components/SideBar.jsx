...
//                 cursor-pointer naz-sm:text-sm${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
...
//                    </div>
// {index > 2 && (
//   <p className='absolute top-4 right-4 text-xs h-7 w-7 flex justify-center items-center rounded-full bg-violet-700/50'>
//      {index}
//   </p>
// )}
// </div>
...
const SideBar = () => {
  ...
  return (
    <div
      ...
      `}
    >
      {/* --- Header Section: Logo and Dropdown Menu --- */}
      <div className="pb-5">
        <div className="flex justify-between items-center">
          {/* Logo */}
          <img src={assets.logo} alt="logo" className="max-w-40" />
        ...
        {filteredUsers.map((user, index) => (
          <div onClick={() => { setSelectedUser(user); setUnseenMessages(prev => ({
            ...
            className={`
              ...
              ${selectedUser?._id === user._id ? 'hover:bg-[#282142]/50' : ''}
            `}
          >
            {/* User Avatar */}
            <img
